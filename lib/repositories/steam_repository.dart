...
class SteamRepository {
  ...
  Future<String> getUsername(String steamId) async {
    final url = 'https://steamcommunity.com/profiles/$steamId/?xml=1';
    try {
      final response = await dioClient.dio.get(url);
      final document = XmlDocument.parse(response.data);
      final unescape = HtmlUnescape();
      final steamIDElement = document.findAllElements('steamID').first;

      String username = steamIDElement.innerText.trim();
      String decodedUsername = unescape.convert(username);

      return decodedUsername;
    } catch (e) {
      throw Exception("Erreur lors de la récupération du nom d'utilisateur: $e");
    }
  }
}
